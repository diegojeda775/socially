...
import { toggleFollow, User } from "@/app/actions/user.action"
...
function ProfilePageClient(props: ProfileProps) {
  const { posts, likedPosts, isFollowing: initialIsFollowing, user, currentUser} = props
...
const isBusy = useMemo(() => isUpdatingFollow, [isUpdatingFollow])
        ...
        <Tabs defaultValue="posts" className="w-full">
          ...
          <TabsContent value="posts" className="mt-6">
            <div className="space-y-6">
              {posts.length > 0 ? (
                posts.map((post) => <PostCard key={post.id} post={post} dbUserId={user.id} user={currentUser}/>)
              ) : (
          ...
          <TabsContent value="likes" className="mt-6">
            <div className="space-y-6">
              {likedPosts.length > 0 ? (
                likedPosts.map((post) => <PostCard key={post.id} post={post} dbUserId={user.id} user={currentUser}/>)
              ) : (
